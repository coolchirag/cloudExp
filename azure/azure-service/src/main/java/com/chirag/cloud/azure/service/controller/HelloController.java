...
public class HelloController {
    ...
    String hello() throws Exception {
    	ServiceBusReceiverAsyncClient serviceBusReceiverClientAsync = sbs.getServiceBusReceiverClientAsync("temp_test_queue");
    	//ServiceBusReceiverClient serviceBusReceiverClient = sbs.getServiceBusReceiverClient("temp_test_queue");
        Flux<ServiceBusReceivedMessage> receiveQueueMessageAsync = sbs.receiveQueueMessageAsync(serviceBusReceiverClientAsync);
    ...
    String hello2() throws Exception {
    	ServiceBusReceiverClient serviceBusReceiverClient = sbs.getServiceBusReceiverClient("temp_test_queue");
    	IterableStream<ServiceBusReceivedMessage> receiveQueueMessage = sbs.receiveQueueMessage(serviceBusReceiverClient);
        	...
        	System.out.println(msgBody);
				try {
					for(int i =1;i<=8; i++) {
						System.out.println("Start Wait time : "+i+" minutes is started.");
						Thread.sleep(60*1000);
						System.out.println("Wait time : "+i+" minutes is completed.");
					}
				//	t1.interrupt();
